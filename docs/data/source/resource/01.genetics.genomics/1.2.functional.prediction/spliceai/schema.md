...
|------|------------|---------|
| `DS_AG` | Delta score for acceptor gain; probability (0-1) that variant creates a new splice acceptor site | `0.85` |
| `DS_AL` | Delta score for acceptor loss; probability (0-1) that variant disrupts an existing splice acceptor | `0.12` |
| `DS_DG` | Delta score for donor gain; probability (0-1) that variant creates a new splice donor site | `0.91` |
| `DS_DL` | Delta score for donor loss; probability (0-1) that variant disrupts an existing splice donor | `0.08` |
| `DP_AG` | Delta position for acceptor gain; distance in base pairs from variant to predicted splice change | `-28` |
| `DP_AL` | Delta position for acceptor loss; distance in base pairs from variant to predicted splice change | `-31` |
| `DP_DG` | Delta position for donor gain; distance in base pairs from variant to predicted splice change | `2` |
| `DP_DL` | Delta position for donor loss; distance in base pairs from variant to predicted splice change | `-31` |
| `max_delta` | Maximum delta score across all four splicing mechanisms for a variant | `0.91` |
| `transcript_priority` | Transcript classification indicating clinical/functional importance | `MANE Select` |
| `distance` | Maximum base pair distance from variant to search for splice site effects | `500` |
| `mask` | Option to exclude scores at positions overlapping reference splice sites | `0` (unmasked) |

### Domain-Specific Terms

| Term | Definition | Related To |
|------|------------|------------|
| Splice Acceptor | 3' end of an intron where splicing occurs; typically contains AG dinucleotide | DS_AG, DS_AL |
| Splice Donor | 5' end of an intron where splicing occurs; typically contains GT dinucleotide | DS_DG, DS_DL |
| Cryptic Splicing | Aberrant splicing at non-canonical sites activated by genetic variants | Delta Scores |
| MANE Select | Matched Annotation from NCBI and EMBL-EBI; primary transcript for clinical reporting | transcript_priority |
| MANE Plus Clinical | Additional clinically relevant transcripts beyond MANE Select | transcript_priority |
| VUS | Variant of Uncertain Significance; variant with unknown clinical impact | Clinical Use Cases |

### Acronyms

| Acronym | Expansion | Notes |
|---------|-----------|-------|
| SpliceAI | Splice-site Artificial Intelligence | Deep learning model for splice prediction |
| VCF | Variant Call Format | Standard file format for genetic variants |
| HGVS | Human Genome Variation Society | Nomenclature for describing sequence variants |
| SNV | Single Nucleotide Variant | Single base pair substitution |
| bp | Base Pairs | Unit of DNA length |
| GENCODE | ENCODE Gene Annotation | Reference gene annotation database |
| MANE | Matched Annotation from NCBI and EBI | Consensus transcript set |
| GTEx | Genotype-Tissue Expression | Gene expression database |
| OMIM | Online Mendelian Inheritance in Man | Genetic disorder database |
