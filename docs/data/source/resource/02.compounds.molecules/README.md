---
id: compounds.molecules
title: "Compounds and Molecules"
type: category
parent: ../README.md
last_updated: 2026-01-23
status: active
tags: [compounds, molecules, drugs, natural-products, pharmacology, chemistry]
---

# Compounds and Molecules

**Parent:** [Data Sources](../README.md)

## Overview

This category encompasses databases focused on chemical compounds, including natural products, pharmaceuticals, food compounds, and their interactions with biological targets. These resources provide essential chemical structure, bioactivity, and pharmacological data for drug discovery, natural product research, and nutritional science.

Chemical compound databases range from comprehensive repositories like PubChem and ChEMBL to specialized collections of natural products (LOTUS, COCONUT), traditional medicine compounds (linked to Category 5), and food-derived molecules. They support compound identification, property prediction, and target interaction analysis.

The integration of compound data with biological targets, pathways, and clinical outcomes enables rational drug design, polypharmacology analysis, and understanding of structure-activity relationships across diverse chemical spaces.

## Subcategories

| Subcategory | Description | Sources |
|-------------|-------------|---------|
| [2.1 Natural Products](./2.1.natural.products/README.md) | Plant, microbial, and marine natural products | 5 |
| [2.2 Pharmaceuticals](./2.2.pharmaceuticals/README.md) | Drug databases and pharmaceutical information | 5 |
| [2.3 Traditional Medicine Compounds](./2.3.traditional.medicine.compounds/README.md) | Compounds from traditional medicine systems | 0 |
| [2.4 Food Compounds & Nutrients](./2.4.food.compounds.nutrients/README.md) | Dietary compounds and nutritional molecules | 3 |
| [2.5 Drug Metabolism & Pharmacokinetics](./2.5.drug.metabolism.pharmacokinetics/README.md) | ADMET and metabolism prediction | 2 |
| [2.6 Chemical Ontology & Classification](./2.6.chemical.ontology.classification/README.md) | Chemical classification and ontologies | 4 |
| [2.7 Compound-Target Interactions](./2.7.compound.target.interactions/README.md) | Binding affinity and interaction databases | 4 |

## Key Statistics

| Metric | Value |
|--------|-------|
| Total Sources | 23 |
| Tier 1 Sources | 12 |
| With Schemas | 18 |

## Key Integration Points

- **Genetics**: Link compounds to pharmacogenomic variants via PharmGKB
- **Pathways**: Connect to metabolic pathways via KEGG, Reactome
- **Diseases**: Associate compounds with indications via DrugBank, ChEMBL
- **Traditional Medicine**: Cross-reference with TCM, Ayurveda databases
- **Proteins**: Map to target structures via UniProt, PDB

## Research Applications

1. Drug discovery and lead optimization
2. Natural product identification and dereplication
3. Drug-drug interaction prediction
4. ADMET property prediction
5. Target identification and validation
6. Structure-activity relationship analysis

## See Also

- [Traditional Medicine](../05.traditional.medicine/README.md)
- [Nutrition & Food Sciences](../06.nutrition.food/README.md)
- [Pathways & Networks](../04.pathways.networks/README.md)
