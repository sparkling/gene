...
|------|------------|---------|
| `studyId` | Unique identifier for a cancer genomics study | `gbm_tcga_pub` |
| `cancerTypeId` | Identifier linking to hierarchical cancer taxonomy | `difg` |
| `molecularProfileId` | Identifier for a specific molecular data type within a study | `gbm_tcga_pub_mutations` |
| `entrezGeneId` | NCBI Entrez Gene database identifier | `1` |
| `hugoGeneSymbol` | Official gene symbol from HUGO Gene Nomenclature Committee | `A1BG` |
| `molecularAlterationType` | Category of molecular change being measured | `MUTATION_EXTENDED` |
| `datatype` | Format or processing level of molecular data | `MAF` |
| `clinicalAttributeId` | Standardized identifier for clinical data fields | `AGE` |
| `Variant_Classification` | Functional consequence of a mutation | `Missense_Mutation` |
| `referenceGenome` | Human genome assembly version used for coordinates | `hg38` |

### Domain-Specific Terms

| Term | Definition | Related To |
|------|------------|------------|
| Somatic Mutation | Genetic alteration acquired in tumor cells, not inherited | molecularAlterationType |
| Copy Number Alteration | Change in the number of copies of a genomic region | COPY_NUMBER_ALTERATION |
| Structural Variant | Large-scale genomic rearrangement (>50bp) | STRUCTURAL_VARIANT |
| Molecular Profile | Collection of molecular measurements for samples in a study | molecularProfileId |
| Clinical Attribute | Patient or sample characteristic recorded in study | clinicalAttributeId |
| Z-Score | Normalized expression value relative to reference samples | Expression Data Types |

### Acronyms

| Acronym | Expansion | Notes |
|---------|-----------|-------|
| MAF | Mutation Annotation Format | Standard mutation file format |
| TCGA | The Cancer Genome Atlas | NCI cancer genomics program |
| CNV | Copy Number Variation | Genomic amplification or deletion |
| HUGO | Human Genome Organisation | Gene nomenclature authority |
| PMID | PubMed Identifier | Publication reference number |
| RPPA | Reverse Phase Protein Array | Protein quantification method |
| SNP | Single Nucleotide Polymorphism | Single base variation |
