...
databases:
  ...
  - dbnsfp
  - alphamissense
tags: [genetics, population, annotation, variants]
---
...
|------|------------|---------|
| `rsID` | Reference SNP identifier assigned by dbSNP to uniquely identify a genetic variant | rs1801133 |
| `allele frequency` | The proportion of a specific allele in a population, expressed as a decimal or percentage | 0.25 (25% of alleles are the variant) |
| `pathogenicity score` | A computational prediction of whether a variant is likely to cause disease | REVEL score of 0.85 indicates likely pathogenic |
| `PHRED score` | A logarithmic quality score where higher values indicate greater confidence in deleteriousness | CADD PHRED >20 = top 1% most deleterious |
| `cCRE` | Candidate cis-regulatory element - a genomic region predicted to regulate nearby gene expression | ENCODE cCRE with enhancer-like signature |
| `structural variant (SV)` | A genomic alteration larger than 50 bp including deletions, duplications, inversions, and insertions | 500 bp deletion in BRCA1 |
| `freeze` | A versioned snapshot of a sequencing consortium's data release | TOPMed Freeze 10 |

### Domain-Specific Terms

| Term | Definition | Related To |
|------|------------|------------|
| `gnomAD` | Genome Aggregation Database - population allele frequencies from 807K+ individuals | Population frequencies |
| `dbNSFP` | Database of functional predictions for non-synonymous SNPs containing 35+ prediction scores | Functional annotation |
| `AlphaMissense` | DeepMind's deep learning model predicting pathogenicity of 71M missense variants | Pathogenicity prediction |
| `SpliceAI` | Deep learning model predicting splice-altering effects of genetic variants | Splice prediction |
| `CADD` | Combined Annotation Dependent Depletion - ensemble deleteriousness score | Functional annotation |
| `VEP` | Variant Effect Predictor - Ensembl tool for annotating variants with multiple databases | Annotation pipeline |
| `MAVE` | Multiplexed Assay of Variant Effect - experimental functional characterization of variants | MaveDB |
| `ChromHMM` | Chromatin state model segmenting the genome into functional categories | Epigenetics |

### Acronyms

| Acronym | Expansion | Notes |
|---------|-----------|-------|
| SNV | Single Nucleotide Variant | Point mutation at a single base |
| CNV | Copy Number Variant | Deletion or duplication affecting gene dosage |
| WGS | Whole Genome Sequencing | Complete genome sequencing (30x+ coverage typical) |
| WES | Whole Exome Sequencing | Sequencing of protein-coding regions only |
| MAF | Minor Allele Frequency | Frequency of less common allele in population |
| ENCODE | Encyclopedia of DNA Elements | NIH consortium for regulatory element annotation |
| TOPMed | Trans-Omics for Precision Medicine | NHLBI sequencing program with diverse populations |
| ALFA | Allele Frequency Aggregator | NCBI's population frequency database |
| VDS | Variant Dataset | Hail format for storing genetic variants |
| HGSVC | Human Genome Structural Variation Consortium | Reference SV catalog from long-read sequencing |
